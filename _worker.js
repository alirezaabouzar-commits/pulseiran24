/* build: v84 — فیلتر پست‌های بی‌محتوا از آرشیو، sitemap و rss */
/* ============================================================
...
/* ============================================================================
   ...
   ============================================================================ */
/* =====================================================================
   سیاههٔ تصحیح‌ها — Corrections Log module  (append-only, target v84)
   Pulse Iran 24
   ---------------------------------------------------------------------
   طراحی‌شده برای صرفه‌جویی در KV (سقف ۱۰۰۰ put روزانه):
     • همهٔ تصحیح‌ها در یک کلید JSON نگه داشته می‌شوند: corr_log
     • هر تصحیح تازه = ۱ put (نه به ازای هر فیلد یا هر بازدید)
     • خواندن از Cache API لبه (۵ دقیقه) → صفر خواندن KV در حالت عادی
     • بازدید صفحه هیچ نوشتنی در KV ایجاد نمی‌کند

   این ماژول کامل و مستقل است: از هیچ helper موجودی در _worker.js
   استفاده نمی‌کند (نه escHtml، نه formatFaDate، نه checkAdmin)، پس
   خطر تعریف تکراری در merge صفر است. همهٔ نام‌ها با corr پیشوند دارند.
   ---------------------------------------------------------------------

   ── دو ویرایش دستی لازم ──

   ۱) در _worker.js، بالای fallback مربوط به ASSETS این سه خط را اضافه کن:

        if (path === '/api/corrections')  return corrApi(request, env);
        if (path === '/tashih-log-admin') return corrAdminPage(request, env);
        if (path === '/tashih' || path === '/tashih.html')
                                          return corrTashihPage(request, env);

   ۲) در tashih.html، هرجا می‌خواهی سیاهه نمایش داده شود (پیشنهاد: پایین
      صفحه، بعد از جدول سه‌سطحی) این بلوک را بگذار:

        <section id="corrLogSection">
          <h2>سیاههٔ تصحیح‌ها</h2>
          <p>هر تصحیحی که روی مطالب منتشرشده اعمال شده، اینجا ثبت می‌شود.</p>
          <div id="corrLog"><!--CORR_LOG--></div>
        </section>
        <script>
        // fallback سمت کلاینت: اگر worker چیزی تزریق نکرد
        (function(){
          var el = document.getElementById('corrLog');
          if (!el || el.querySelector('.corr-item')) return;
          fetch('/api/corrections').then(function(r){return r.json();})
            .then(function(d){ if (d && d.html) el.innerHTML = d.html; })
            .catch(function(){});
        })();
        </script>

   (اگر _worker.js را برایم بفرستی، هر دو ویرایش را انجام می‌دهم و فایل
    کامل تست‌شده را برای آپلود پس می‌فرستم — دست‌کاری دستی لازم نیست.)
   ===================================================================== */

const CORR_KEY        = 'corr_log';
const CORR_MAX        = 200;          // سقف نگهداری
const CORR_EDGE_TTL   = 300;          // ثانیه — کش لبه
const CORR_EDGE_URL   = 'https://pulseiran24.com/__edge/corrections';

const CORR_LEVELS = {
  1: { icon: '🔧', label: 'اصلاح جزئی',      cls: 'corr-lv1' },
  2: { icon: '⚠️', label: 'اصلاح محتوایی',   cls: 'corr-lv2' },
  3: { icon: '❌', label: 'پس‌گرفته شد',      cls: 'corr-lv3' }
};

/* ---------- helpers (همه با پیشوند corr) ---------- */
...
const CORR_FA_MONTHS = ['فروردین','اردیبهشت','خرداد','تیر','مرداد','شهریور',
                        'مهر','آبان','آذر','دی','بهمن','اسفند'];

// تاریخ ورودی شمسی است (مثل 1405-05-08) یا میلادی ISO
function corrFaDate(v) {
  if (!v) return '';
  const m = String(v).match(/^(\d{4})-(\d{1,2})-(\d{1,2})/);
  if (!m) return String(v);
  const y = +m[1], mo = +m[2], d = +m[3];
  if (y > 1300 && y < 1500) {
    const name = CORR_FA_MONTHS[mo - 1] || mo;
    return corrFaDigits(`${d} ${name} ${y}`);
  }
  try {
...
function corrAuthed(request, env) {
  const token = env.ADMIN_TOKEN;
  if (!token) return false;
  const url = new URL(request.url);
  const h = request.headers.get('x-admin-token') || url.searchParams.get('token') || '';
  return h === token;
}

function corrJson(obj, status) {
  return new Response(JSON.stringify(obj), {
    status: status || 200,
...
async function corrLoad(env) {
  // ۱) کش لبه
  try {
    const hit = await caches.default.match(CORR_EDGE_URL);
    if (hit) return await hit.json();
  } catch (e) {}
  // ۲) KV
  let list = [];
  try {
    const raw = await env.PULSE_STATS.get(CORR_KEY);
    if (raw) list = JSON.parse(raw) || [];
  } catch (e) { list = []; }
  if (!Array.isArray(list)) list = [];
  // ۳) نوشتن در کش لبه (بدون KV write)
  try {
    await caches.default.put(CORR_EDGE_URL, new Response(JSON.stringify(list), {
      headers: {
        'content-type': 'application/json; charset=utf-8',
        'cache-control': `public, max-age=${CORR_EDGE_TTL}`
      }
    }));
  } catch (e) {}
  return list;
...
async function corrSave(env, list) {
  const trimmed = list.slice(0, CORR_MAX);
  await env.PULSE_STATS.put(CORR_KEY, JSON.stringify(trimmed));   // ۱ put
  try { await caches.default.delete(CORR_EDGE_URL); } catch (e) {}
  return trimmed;
}
...
function corrStyles() {
  return `<style>
#corrLogSection .corr-wrap{margin:1.5rem 0;display:flex;flex-direction:column;gap:.9rem}
.corr-item{border:1px solid rgba(128,128,128,.28);border-radius:12px;padding:1rem 1.1rem;
  background:rgba(128,128,128,.06);position:relative}
.corr-item{border-inline-start:4px solid rgba(128,128,128,.45)}
.corr-item.corr-lv1{border-inline-start-color:#3b82f6}
.corr-item.corr-lv2{border-inline-start-color:#f59e0b}
.corr-item.corr-lv3{border-inline-start-color:#dc2626}
.corr-head{display:flex;flex-wrap:wrap;gap:.5rem .75rem;align-items:baseline;
  margin-bottom:.6rem;font-size:.92rem}
.corr-badge{font-weight:700}
.corr-item.corr-lv1 .corr-badge{color:#2563eb}
.corr-item.corr-lv2 .corr-badge{color:#b45309}
.corr-item.corr-lv3 .corr-badge{color:#b91c1c}
.corr-date{opacity:.7}
.corr-title{font-weight:700;margin:0 0 .5rem;font-size:1.02rem;line-height:1.6}
.corr-title a{text-decoration:none;border-bottom:1px dotted currentColor}
.corr-row{margin:.35rem 0;line-height:1.85;font-size:.95rem}
.corr-row b{opacity:.75;font-weight:600}
.corr-wrong{text-decoration:line-through;opacity:.75}
.corr-src{font-size:.88rem;opacity:.75;margin-top:.55rem}
.corr-empty{opacity:.7;padding:1rem 0}
@media (max-width:520px){.corr-item{padding:.85rem}}
</style>`;
}

function corrItemHtml(it) {
  const lv = CORR_LEVELS[it.level] || CORR_LEVELS[2];
  const title = it.articleUrl
    ? `<a href="${corrEsc(it.articleUrl)}">${corrEsc(it.articleTitle || it.articleUrl)}</a>`
    : corrEsc(it.articleTitle || '—');

  const rows = [];
  if (it.wrong) rows.push(
    `<p class="corr-row"><b>آنچه منتشر شده بود:</b> <span class="corr-wrong">${corrEsc(it.wrong)}</span></p>`);
  if (it.right) rows.push(
    `<p class="corr-row"><b>متن درست:</b> ${corrEsc(it.right)}</p>`);
  if (it.note) rows.push(
    `<p class="corr-row">${corrEsc(it.note)}</p>`);

  return `<article class="corr-item ${lv.cls}" id="corr-${corrEsc(it.id)}">
  <div class="corr-head">
    <span class="corr-badge">${lv.icon} ${lv.label}</span>
    <span class="corr-date">${corrEsc(corrFaDate(it.date))}</span>
  </div>
  <h3 class="corr-title">${title}</h3>
  ${rows.join('\n  ')}
  ${it.source ? `<p class="corr-src">مبنای تصحیح: ${corrEsc(it.source)}</p>` : ''}
</article>`;
}

function corrListHtml(list) {
  if (!list.length) {
    return `${corrStyles()}<p class="corr-empty">تا این لحظه تصحیحی ثبت نشده است. هر تصحیح آینده در همین بخش منتشر می‌شود.</p>`;
  }
  return `${corrStyles()}<div class="corr-wrap">
${list.map(corrItemHtml).join('\n')}
</div>`;
}

/* ---------- routes ---------- */

// GET  /api/corrections            → { count, items, html }
// POST /api/corrections  (admin)   → افزودن   { level, date, articleTitle, articleUrl, wrong, right, source, note }
// POST /api/corrections?delete=ID  → حذف
async function corrApi(request, env) {
  if (request.method === 'GET') {
    const list = await corrLoad(env);
    return new Response(JSON.stringify({
      count: list.length, items: list, html: corrListHtml(list)
    }), {
      headers: {
        'content-type': 'application/json; charset=utf-8',
        'cache-control': `public, max-age=${CORR_EDGE_TTL}`
  ...
  if (request.method !== 'POST') return corrJson({ error: 'method' }, 405);
  if (!corrAuthed(request, env)) return corrJson({ error: 'unauthorized' }, 401);

  const url = new URL(request.url);
  const del = url.searchParams.get('delete');
  let list = await corrLoad(env);

  if (del) {
    const before = list.length;
    list = list.filter(x => x.id !== del);
    if (list.length === before) return corrJson({ error: 'not found' }, 404);
    await corrSave(env, list);
    return corrJson({ ok: true, deleted: del, count: list.length });
  }

  let body;
  try { body = await request.json(); }
  catch (e) { return corrJson({ error: 'bad json' }, 400); }

  const level = [1, 2, 3].includes(+body.level) ? +body.level : 2;
  if (!body.articleTitle && !body.articleUrl)
    return corrJson({ error: 'عنوان یا نشانی مطلب لازم است' }, 400);
  if (!body.right && !body.note)
    return corrJson({ error: 'متن درست یا توضیح لازم است' }, 400);
  ...
  list.unshift(entry);
  const saved = await corrSave(env, list);
  return corrJson({ ok: true, id: entry.id, count: saved.length });
}
...
async function corrTashihPage(request, env) {
  ...
  try {
    ...
    return env.ASSETS.fetch(request);          // fail-open
  }
  ...
  try {
    const list = await corrLoad(env);
    block = corrListHtml(list);
  } catch (e) {
    block = '';                                // fail-open: صفحه سالم می‌ماند
  }

  const out = html.replace('<!--CORR_LOG-->', block);
...
// پنل ادمین: /tashih-log-admin
async function corrAdminPage(request, env) {
  return new Response(`<!DOCTYPE html>
<html lang="fa" dir="rtl"><head>
<meta charset="utf-8">
<meta name="viewport" content="width=device-width,initial-scale=1">
<meta name="robots" content="noindex,nofollow">
<title>سیاههٔ تصحیح‌ها — مدیریت</title>
<link rel="stylesheet" href="/assets/fonts/vazirmatn.css">
<style>
:root{--bg:#0f1115;--card:#171a21;--line:#2a2f3a;--fg:#e8eaed;--muted:#9aa3af;--acc:#dc2626}
*{box-sizing:border-box}
body{margin:0;background:var(--bg);color:var(--fg);font-family:Vazirmatn,Tahoma,sans-serif;
  padding:1.2rem;line-height:1.7}
.wrap{max-width:760px;margin:0 auto}
h1{font-size:1.3rem;margin:0 0 1rem}
.card{background:var(--card);border:1px solid var(--line);border-radius:14px;padding:1.1rem;margin-bottom:1.1rem}
label{display:block;font-size:.9rem;color:var(--muted);margin:.7rem 0 .3rem}
input,textarea,select{width:100%;padding:.6rem .7rem;background:#0c0e12;color:var(--fg);
  border:1px solid var(--line);border-radius:9px;font-family:inherit;font-size:.95rem}
textarea{min-height:72px;resize:vertical}
button{background:var(--acc);color:#fff;border:0;border-radius:9px;padding:.7rem 1.2rem;
  font-family:inherit;font-size:.98rem;font-weight:700;cursor:pointer;margin-top:1rem}
button.ghost{background:transparent;border:1px solid var(--line);color:var(--muted);
  font-weight:400;font-size:.85rem;padding:.35rem .7rem;margin:0}
#msg{margin-top:.8rem;font-size:.92rem;min-height:1.4em}
.ok{color:#34d399}.err{color:#f87171}
.row{display:flex;gap:.8rem;flex-wrap:wrap}.row>div{flex:1;min-width:150px}
.it{border-top:1px solid var(--line);padding:.8rem 0;font-size:.9rem}
.it:first-child{border-top:0}
.it b{display:block;margin-bottom:.25rem}
.it .m{color:var(--muted);font-size:.84rem}
.hint{color:var(--muted);font-size:.85rem;margin:.2rem 0 0}
</style></head><body><div class="wrap">
<h1>سیاههٔ تصحیح‌ها</h1>

<div class="card">
  <label>رمز مدیریت</label>
  <input id="tok" type="password" placeholder="ADMIN_TOKEN" autocomplete="current-password">
  <p class="hint">در مرورگر ذخیره می‌شود تا هر بار وارد نکنی.</p>
</div>

<div class="card">
  <div class="row">
    <div>
      <label>سطح تصحیح</label>
      <select id="level">
        <option value="1">۱ — اصلاح جزئی (املا، نام، تاریخ فرعی)</option>
        <option value="2" selected>۲ — اصلاح محتوایی (عدد، منبع، نقل‌قول)</option>
        <option value="3">۳ — خبر نادرست بود و پس گرفته شد</option>
      </select>
    </div>
    <div>
      <label>تاریخ (۱۴۰۵-۰۵-۰۸ یا خالی برای امروز)</label>
      <input id="date" placeholder="1405-05-08">
    </div>
  </div>

  <label>عنوان مطلب</label>
  <input id="articleTitle" placeholder="تیتر مطلبی که تصحیح شده">

  <label>نشانی مطلب</label>
  <input id="articleUrl" placeholder="/news/12345 یا /tahlil/1405-05-07" dir="ltr">

  <label>آنچه منتشر شده بود (نادرست)</label>
  <textarea id="wrong" placeholder="گزارهٔ نادرست، عیناً"></textarea>

  <label>متن درست</label>
  <textarea id="right" placeholder="گزارهٔ درست"></textarea>

  <label>مبنای تصحیح (منبع)</label>
  <input id="source" placeholder="مثلاً: بیانیهٔ رسمی، گزارش رویترز ۸ مرداد">

  <label>توضیح اختیاری</label>
  <textarea id="note" placeholder="اگر لازم است"></textarea>

  <button id="save">ثبت تصحیح</button>
  <div id="msg"></div>
</div>

<div class="card">
  <b>ثبت‌شده‌ها</b>
  <div id="list" class="hint">در حال بارگذاری…</div>
</div>
</div>
<script>
const $=id=>document.getElementById(id);
$('tok').value=localStorage.getItem('pi24_admin')||'';
$('tok').onchange=()=>localStorage.setItem('pi24_admin',$('tok').value);

const LV={1:'🔧 جزئی',2:'⚠️ محتوایی',3:'❌ پس‌گرفته'};

async function load(){
  try{
    const r=await fetch('/api/corrections',{cache:'no-store'});
    const d=await r.json();
    if(!d.items||!d.items.length){$('list').textContent='هنوز چیزی ثبت نشده.';return;}
    $('list').innerHTML=d.items.map(it=>
      '<div class="it"><b>'+(LV[it.level]||'')+' — '+esc(it.articleTitle||it.articleUrl||'—')+'</b>'+
      '<span class="m">'+esc(it.date||'')+'</span> '+
      '<button class="ghost" data-id="'+it.id+'">حذف</button></div>').join('');
    $('list').querySelectorAll('button[data-id]').forEach(b=>b.onclick=()=>del(b.dataset.id));
...
$('save').onclick=async()=>{
  ...
  if(!tok){msg('رمز را وارد کن.',false);return;}
  const body={};
  ['level','date','articleTitle','articleUrl','wrong','right','source','note']
    .forEach(k=>body[k]=$(k).value.trim());
  $('save').disabled=true;
  try{
    const r=await fetch('/api/corrections',{method:'POST',
      headers:{'content-type':'application/json','x-admin-token':tok},
      body:JSON.stringify(body)});
    const d=await r.json();
    if(d.ok){
      msg('ثبت شد.',true);
      ['articleTitle','articleUrl','wrong','right','source','note','date']
        .forEach(k=>$(k).value='');
      load();
    } else msg(d.error||'خطا',false);
  }catch(e){msg('خطای شبکه',false);}
  $('save').disabled=false;
};
...
async function del(id){
  ...
  const tok=$('tok').value.trim();
  try{
    const r=await fetch('/api/corrections?delete='+encodeURIComponent(id),
      {method:'POST',headers:{'x-admin-token':tok}});
    const d=await r.json();
    if(d.ok){msg('حذف شد.',true);load();} else msg(d.error||'خطا',false);
  }catch(e){msg('خطای شبکه',false);}
}
