...
function aiPanelPage() {
  const html = '<!doctype html><html lang="fa" dir="rtl"><head>'
    ...
    + 'html+=\'<button class="p" onclick="pub(\'+i+\')" id="pb\'+i+\'">انتشار روی سایت</button>\';'
    + 'html+=\'<label style="display:flex;align-items:center;gap:5px;font-size:13px;color:var(--dim)">\';'
    + 'html+=\'<input type="checkbox" id="pu\'+i+\'" style="width:16px;height:16px;padding:0"> اعلان وب‌پوش</label>\';'
    + 'html+=\'<button onclick="cp(\\\'ti\'+i+\'\\\',this)">کپی تیتر</button>\';'
    + 'html+=\'<button onclick="cp(\\\'bo\'+i+\'\\\',this)">کپی متن</button>\';'
    + 'html+=\'<button onclick="window.open(\\\'/admin\\\',\\\'_blank\\\')">پنل انتشار</button></div>\';'
    ...
    + 'function pub(i){'
    + 'var t=$("ti"+i).value.trim(),b=$("bo"+i).value.trim();'
    + 'if(!t){alert("تیتر خالی است.");return}'
    + 'if(!confirm("این خبر روی سایت منتشر شود؟\\n\\n"+t)){return}'
    + 'var pb=$("pb"+i);pb.disabled=true;pb.textContent="در حال انتشار...";'
    + 'fetch("/admin/api",{method:"POST",headers:{"content-type":"application/json"},'
    + 'body:JSON.stringify({action:"create",title:t,body:b,push:$("pu"+i).checked,token:TK})})'
    + '.then(function(r){return r.json()}).then(function(d){'
    + 'if(!d.ok){pb.disabled=false;pb.textContent="انتشار روی سایت";'
    + 'alert("انتشار ناموفق بود: "+(d.error||"نامشخص"));return}'
    + 'mark(ITEMS[i].link);'
    + 'pb.textContent="منتشر شد";pb.className="ok";'
    + 'var extra=d.pushed?" (اعلان فرستاده شد)":(d.pushError?" (اعلان نرفت: "+d.pushError+")":"");'
    + '$("o"+i).innerHTML+=\'<div class="note ok">منتشر شد: <a href="\'+esc(d.url)+\'" target="_blank" rel="noopener noreferrer" style="color:#6ee7a0">\'+esc(d.url)+\'</a>\'+esc(extra)+\'</div>\';'
    + '}).catch(function(){pb.disabled=false;pb.textContent="انتشار روی سایت";alert("اتصال ناموفق بود.")})}'
